"""Modelos fisicos simplificados do motor eletrico industrial MTR-001.
...
@dataclass(slots=True)
class Perturbation:
    """Efeito agregado das falhas sobre a fisica nominal do motor."""

    load_multiplier: float = 1.0
    extra_current_a: float = 0.0
    extra_temperature_c: float = 0.0
    extra_vib_velocity: float = 0.0  # mm/s
    extra_vib_accel: float = 0.0  # g
    vib_velocity_gain: float = 1.0
...
class MotorModel:
    ...
    def step(
        ...
        # Carga efetiva: setpoint + leve ondulacao operacional + efeito de falha.
        wander = 0.04 * math.sin(self._load_phase / 37.0)
        load = _clamp((load_setpoint + wander) * pert.load_multiplier, 0.0, 1.5)
        ...
        current = (
            ...
            + pert.extra_current_a
            + random.gauss(0.0, 0.25)
        )

        # Temperatura: modelo termico de primeira ordem (aproximacao exponencial).
        steady = AMBIENT_TEMP_C + MAX_TEMP_RISE_C * load**2 + pert.extra_temperature_c
        ...
        # Vibracao: base + carga + falha (faixas da DIN ISO 10816/20816).
        vib_velocity = (
            (BASE_VIB_VELOCITY_RMS + 0.35 * load) * pert.vib_velocity_gain
            + pert.extra_vib_velocity
            + abs(random.gauss(0.0, 0.06))
        )
        vib_accel = (
            BASE_VIB_ACCEL_RMS
            + 0.05 * load
            + pert.extra_vib_accel
            + abs(random.gauss(0.0, 0.012))
        )
