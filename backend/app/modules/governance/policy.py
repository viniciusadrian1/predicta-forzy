...
ACCESS_RULES: list[AccessRule] = [
    ...
    AccessRule(
        resource="/assets/{tag} (edição)",
        methods=["PATCH"],
        min_role="operator",
        description="Edição de ativos.",
    ),
    AccessRule(
        resource="/assets/{tag} (exclusão)",
        methods=["DELETE"],
        min_role="engineer",
        description="Exclusão de ativos.",
    ),
    AccessRule(
        resource="/assets/{tag}/validate",
        methods=["POST"],
        min_role="engineer",
        description="Validação manual de cadastro por IA (Gestor de Planta ou Admin).",
    ),
    AccessRule(
        resource="/plants, /areas (cadastro)",
        methods=["POST"],
        min_role="operator",
        description="Cadastro de plantas e áreas.",
    ),
    AccessRule(
    ...
    AccessRule(
        resource="/assets/extract-from-image, /automation/register-from-image",
        methods=["POST"],
        min_role="operator",
    ...
    AccessRule(
        ...
        methods=["GET"],
        min_role="engineer",
        description="Catálogo de dados classificado (também acessível ao auditor).",
    ),
    AccessRule(
        resource="/audit, /governance/tag-audit",
        methods=["GET"],
        min_role="admin",
        description="Trilha de auditoria do sistema — acesso ao auditor e ao admin.",
    ),
    AccessRule(
        resource="/users",
        methods=["GET"],
